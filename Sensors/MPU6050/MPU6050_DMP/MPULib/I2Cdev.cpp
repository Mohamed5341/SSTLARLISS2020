...
int8_t I2Cdev::readWords(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint16_t *data, uint16_t timeout)
{
    char command[1];
    command[0]=regAddr;
    char *redData = (char*)malloc(2*length);
    i2c.write(devAddr<<1, command, 1,true);
    i2c.read(devAddr<<1, redData,2*length);
    for(int i=0; i < 2*length; i+=2){
        data[i/2] = (redData[i]<<8) | (redData[i+1]);
    }
    free(redData);
    return length;
}
...
{
    bool status;
    unsigned char *sentData = (unsigned char *)malloc(2*length);
    for(int i=0; i<2*length; i+=2){
        sentData[i] = data[i/2]>>8;
        sentData[i + 1] = data[i/2];
    }
    status=writeBytes(devAddr, regAddr, 2*length, sentData);
    free(sentData);
    return status;
}
